...
function doAiLookup() {
  ...
  .then(function(modelsData) {
    ...
    if (MODELS.length === 0) {
      status.innerHTML = '<p class="msg-error">Kļūda 3 sazinieties ar autoru.</p>';
      return;
    }

    function tryModel(modelIndex) {
      if (modelIndex >= MODELS.length) {
        status.innerHTML = '<p class="msg-error">Kļūda 4 mēģiniet vēlāk.</p>';
        return;
      }
      ...
      .then(function(result) {
        ...
        try {
          ...
          try {
            ...
            status.innerHTML = '<p class="msg-error">Mākslīgais intelekts deva nepareizu atbildi vai savienojuma kļūda.</p>';
            return;
          }
        }

        if (!plant.valid) {
          status.innerHTML = '<p class="msg-error">Augs nav atrasts vai ievadīts nepareizs nosaukums.</p>';
          return;
        }
        ...
        status.innerHTML = '<p class="msg-success">Informācija ielādeta! Atceries mākslīgais intelekts var kļūdīties.</p>';
      })
      .catch(function(err) {
...
function savePlant() {
  ...
  if (!name) { status.innerHTML = '<p class="msg-error">Auga nosaukums ir obligāts.</p>'; return; }
  if (!freq || freq < 1) { status.innerHTML = '<p class="msg-error">Laistīsanas biežums ir obligāts.</p>'; return; }
  ...
  plants.push(plant);
  savePlants(plants);
  status.innerHTML = '<p class="msg-success">Augs saglabāts!</p>';
  clearForm();
